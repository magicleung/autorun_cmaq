...
source ..../dylan/CMAQ/CMAQ-5.2/config_cmaq.csh intel # Revise!!
set CMAQ_HOME = ..../dylan/CMAQ/CMAQ-5.2 # Revise!!
set CMAQ_DATA = ..../dylan/CMAQ/CMAQ-5.2/data # Revise!!

set APPL       = ....  
set CoordName  = .... # 16-character maximum
set GridName   = .... # 16-character maximum

set DataPath   = $CMAQ_DATA
set InMetDir   = ..../dylan/WRF/WRFV3/run  # Revise!!
set InTerDir   = ..../dylan/WRF/WPS  # Revise!!
set InSatDir   = $DataPath/goes
set OutDir     = $CMAQ_DATA/mcip/${GridName}
set ProgDir    = $CMAQ_HOME/PREP/mcip/src
set WorkDir    = $OutDir

rm -rf $CMAQ_DATA/data/mcip/${APPL}
...
set InMetFiles = ( $InMetDir/wrfout_d03_${SYYYY}-${SMM}-${SDD}_${SHH}:00:00 )
echo "wrfout file is:  "
echo $InMetFiles
...
set MCIP_START = ${SYYYY}-${SMM}-${SDD}-${SHH}:00:00.0000
set MCIP_END   = ${EYYYY}-${EMM}-${EDD}-${EHH}:00:00.0000
...
set CTMLAYS="1.000, 0.9922, 0.9844, 0.9766, 0.9689,\
                0.9607, 0.9519, 0.9424, 0.9319, 0.9201,\
                0.9068, 0.8915, 0.8739, 0.8534, 0.8297,\
                0.8023, 0.7709, 0.7355, 0.6961, 0.6531,\
                0.607, 0.5586, 0.5087, 0.4582, 0.4077,\
                0.3581, 0.3097, 0.2632, 0.2186, 0.1763,\
                0.1363, 0.0987, 0.0634, 0.0305, 0.000"
