...
# source ./config_cmaq.csh $compiler
 source ../CMAQ-5.2/config_cmaq.csh $compiler
 cd /home/test/models/CMAQ/autorun_scripts

#> Set General Parameters for Configuring the Simulation
 set VRSN      = v52               #> Code Version
 set PROC      = mpi               #> serial or mpi
 set MECH      = cb05tucl_ae6_aq    #> Mechanism ID
 set EMIS      = MEIC            #> Emission Inventory Details
 set APPL      = case1         #> Application Name (e.g. Gridname)
...
#> Set Working, Input, and Output Directories
 setenv WORKDIR /home/test/models/CMAQ/autorun_scripts       #> Working Directory. Where the runscript is.
 setenv OUTDIR  ${CMAQ_DATA}/output_CCTM                     #> Output Directory
 setenv INPDIR  ${CMAQ_DATA}                                 #> Input Directory
# setenv INPDIR  ${CMAQ_DATA}/SE52BENCH
...
#> Set Start and End Days for looping
 ...
 set START_DATE = ${SYYYY}-${SMM}-${SDD}     #> beginning date (July 1, 2011)
 set END_DATE   = ${EYYYY}-${EMM}-${EDD}     #> ending date    (July 14, 2011)
...
setenv GRID_NAME case1         #> check GRIDDESC file for GRID_NAME options
setenv GRIDDESC $INPDIR/mcip/$rundate/GRIDDESC   #> grid description file
...
setenv PACM_INFILE ${CMAQ_DATA}/procan/pa.inp

#> process analysis global column, row and layer ranges
#> user must check GRIDDESC for validity!
setenv PA_BCOL_ECOL "1 85"
setenv PA_BROW_EROW "1 85"
...
set METpath   = $INPDIR/mcip/$rundate     #> meteorology input directory 
...
set ICBC_CASE = 2018         #> Version label for the ICBCs
set EMIS_CASE = 2012         #> Version Label for the Emissions
...
# =====================================================================
 ...
 #> Initial conditions
  set CGRID_FILENAME = CCTM_CGRID_${RUNID}_${YESTERDAY}
  if ( -f $OUTDIR/cctm/${YESTERDAY}/${CGRID_FILENAME} ) then
    set ICpath = ${OUTDIR}/cctm/${YESTERDAY}
    set ICFILE = ${CGRID_FILENAME}
  else
    set ICpath = $INPDIR/icon
    set ICFILE = ICON_v52_${GRID_NAME}_profile
    setenv INITIAL_RUN N
  endif

  #> Boundary conditions
  set BCFILE = BCON_v52_${GRID_NAME}_profile
  ...
  if ( $CTM_PT3DEMIS == 'N' ) then
     ...
     set EMISfile  = emis_MEIC_${GRID_NAME}_${TODAYJ}.ncf
  ...
  setenv OCEAN_1 $EMISpath/Ocean_${GRID_NAME} #> horizontal grid-dependent surf zone file
